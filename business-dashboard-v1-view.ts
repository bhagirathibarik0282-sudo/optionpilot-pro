...
export function renderBusinessDashboardV1Html(model: BusinessDashboardV1Model): string {
  ...
  const intelligence = model.intelligence.map((x)=>`<div class="intel"><span>${esc(x.label)}</span><b class="${x.state === "VERIFIED" ? "buy" : "wait"}">${esc(x.state)}</b><small>${esc(x.detail)}</small></div>`).join("");
  ...
  *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--t);font-family:Inter,system-ui,sans-serif}.wrap{max-width:980px;margin:auto;padding:14px}
  ...
  .section{margin-top:10px}.row{display:flex;gap:8px;flex-wrap:wrap}.intelgrid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px}.intel{background:#08111f;border-radius:12px;padding:10px}.intel span,.intel b,.intel small{display:block}.intel span{font-size:10px;font-weight:900}.intel b{font-size:10px;margin-top:5px}.intel small{font-size:9px;color:var(--m);margin-top:5px;line-height:1.35}.pill{font-size:9px;border:1px solid var(--l);border-radius:999px;padding:5px 8px;color:var(--m)}.meta{font-size:11px;color:var(--m);line-height:1.5}
  @media(max-width:720px){.grid,.intelgrid{grid-template-columns:1fr}.title{font-size:21px}}
    ...
    <section class="card section"><div class="ey">BUSINESS INTELLIGENCE</div><div class="intelgrid">${intelligence}</div></section>
