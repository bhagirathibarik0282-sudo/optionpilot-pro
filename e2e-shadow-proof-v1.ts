...
export interface E2EShadowProofInput {
  ...
  telegramDecisionId: string | null;
  dashboardDecisionId: string | null;
...
export function buildE2EShadowProof(input:E2EShadowProofInput) {
  ...
  if (!input?.canonicalDecisionId) blockers.push("CANONICAL_DECISION_ID_REQUIRED");
  if (input?.canonicalDecisionId !== input?.telegramDecisionId) blockers.push("TELEGRAM_DECISION_ID_MISMATCH");
  if (input?.canonicalDecisionId !== input?.dashboardDecisionId) blockers.push("DASHBOARD_DECISION_ID_MISMATCH");
  ...
  if (input?.shadow?.decisionId !== input?.canonicalDecisionId) blockers.push("KITE_SHADOW_NOT_SAME_DECISION");
  ...
  if (input?.readiness?.decisionId !== input?.canonicalDecisionId) blockers.push("READINESS_NOT_SAME_DECISION");
  ...
  return {
    ...
    symbol:input?.symbol ?? null,observedAt:input?.observedAt ?? null,decisionId:input?.canonicalDecisionId ?? null,candidateKey:input?.canonicalCandidateKey ?? null,
