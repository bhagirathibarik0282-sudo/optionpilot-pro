...
export async function startH1DynamicReadOnlyLiveChain(asOfDate:string, enabled:boolean): Promise<H1DynamicReadOnlyLiveStartResult> {
  ...
  try {
    ...
    const service=new H1LiveExactReadOnlyWebSocketService({readiness,apiKey,accessToken:authority.session.accessToken,constituentRegistry:evidence.constituents.ready ? evidence.constituents.registry : undefined});
