...
export interface H1LiveSelectedContractPair {
...
export function selectH1LiveContracts(rows:KiteInstrumentMasterRow[], spots:H1LiveSelectionSpotRow[], asOfDate:string):H1LiveContractSelectionResult {
  ...
  for(const spot of spots){
    ...
    if(common.length===0) return out(false,[],[`NO_COMMON_CE_PE_STRIKE:${spot.symbol}:${expiry}`]);
    let strike=common[0];
    for(const candidate of common){
      const d=Math.abs(candidate-spot.ltp), best=Math.abs(strike-spot.ltp);
      if(d<best || (d===best && candidate<strike)) strike=candidate;
    }
    const pair=byStrike.get(strike)!;
    selected.push({ symbol:spot.symbol, expiry, spot:spot.ltp, strike, ceInstrumentToken:pair.ce!.instrument_token, peInstrumentToken:pair.pe!.instrument_token, ceTradingsymbol:pair.ce!.tradingsymbol, peTradingsymbol:pair.pe!.tradingsymbol });
  }
