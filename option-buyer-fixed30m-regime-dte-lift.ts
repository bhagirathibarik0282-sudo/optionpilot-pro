...
export interface Fixed30mRegimeDteResult {
 ...
 regimePolicy:"MARKET_REGIME_ENGINE_V1_EXACT_SIGNAL_TIMESTAMP_BOUND"; dtePolicy:"FIXED_DTE_BUCKETS_0_1_2_4_5_7_10_35";
...
const bucket=(dte:number):Fixed30mDteBucket|null=>dte>=0&&dte<=1?"EXPIRY_0_1":dte>=2&&dte<=4?"NEAR_2_4":dte>=5&&dte<=7?"FALLBACK_5_7":dte>=10&&dte<=35?"BANKNIFTY_HIGHER_10_35":null;
...
export function validateFixed30mRegimeDteLift(samples:Fixed30mRegimeDteSample[]):Fixed30mRegimeDteResult{
 ...
 for(const s of samples){
  ...
  const d=bucket(c.dte); if(!d){excludedUnsupportedDte++;continue;}
