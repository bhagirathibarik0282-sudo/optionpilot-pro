...
export interface LiftBucketSummary { dteBucket:LiftDteBucket; baselineCount:number; mdiFilteredCount:number; baselineAvgEstimatedNetReturnPct:number|null; mdiFilteredAvgEstimatedNetReturnPct:number|null; avgNetReturnLiftPctPoints:number|null; baselinePositiveRatePct:number|null; mdiFilteredPositiveRatePct:number|null; positiveRateLiftPctPoints:number|null; }
export interface OptionBuyerLiftValidationResult { state:LiftState; totalSamples:number; baselineQualifiedCount:number; mdiFilteredQualifiedCount:number; buckets:LiftBucketSummary[]; blockers:string[]; ruleVersion:"OPTION_BUYER_MDI_LIFT_VALIDATION_V1"; semantics:"RESEARCH_FILTER_LIFT_ONLY_NOT_CAUSAL_EDGE_PROOF"; baselinePolicy:"SAME_SELECTOR_QUALIFIED_OPTION_BUYER_SAMPLES_WITHOUT_MDI_FILTER"; mdiPolicy:"SAME_SAMPLES_SUBSET_WITH_FULLY_VERIFIED_DIRECTIONAL_MDI_MATCHING_CANDIDATE_SIDE"; payoffPolicy:"CURRENT_RATE_DATE_EXECUTABLE_QUOTE_ESTIMATE_BOUND_TO_CANDIDATE_CONTRACT"; regimePolicy:"NOT_INCLUDED_UNTIL_DETERMINISTIC_REGIME_SOURCE_IS_BOUND"; affectsVerdict:false; affectsTelegram:false; affectsExecution:false; createsOrders:false; aiMayOverride:false; }
...
export function validateOptionBuyerMdiLift(samples:OptionBuyerLiftSample[]):OptionBuyerLiftValidationResult{
 ...
 const buckets=[...byBucket.entries()].map(([dteBucket,g])=>{const bAvg=mean(g.baseline),mAvg=mean(g.mdi),bRate=rate(g.baseline),mRate=rate(g.mdi);return{dteBucket,baselineCount:g.baseline.length,mdiFilteredCount:g.mdi.length,baselineAvgEstimatedNetReturnPct:bAvg,mdiFilteredAvgEstimatedNetReturnPct:mAvg,avgNetReturnLiftPctPoints:bAvg==null||mAvg==null?null:mAvg-bAvg,baselinePositiveRatePct:bRate,mdiFilteredPositiveRatePct:mRate,positiveRateLiftPctPoints:bRate==null||mRate==null?null:mRate-bRate};}).sort((a,b)=>a.dteBucket.localeCompare(b.dteBucket));
 ...
 return{state:blockers.length?"UNAVAILABLE":"USABLE",totalSamples:samples.length,baselineQualifiedCount,mdiFilteredQualifiedCount,buckets,blockers:[...new Set(blockers)],ruleVersion:"OPTION_BUYER_MDI_LIFT_VALIDATION_V1",semantics:"RESEARCH_FILTER_LIFT_ONLY_NOT_CAUSAL_EDGE_PROOF",baselinePolicy:"SAME_SELECTOR_QUALIFIED_OPTION_BUYER_SAMPLES_WITHOUT_MDI_FILTER",mdiPolicy:"SAME_SAMPLES_SUBSET_WITH_FULLY_VERIFIED_DIRECTIONAL_MDI_MATCHING_CANDIDATE_SIDE",payoffPolicy:"CURRENT_RATE_DATE_EXECUTABLE_QUOTE_ESTIMATE_BOUND_TO_CANDIDATE_CONTRACT",regimePolicy:"NOT_INCLUDED_UNTIL_DETERMINISTIC_REGIME_SOURCE_IS_BOUND",affectsVerdict:false,affectsTelegram:false,affectsExecution:false,createsOrders:false,aiMayOverride:false};
}
