...
export interface OptionBuyerRegimeLiftSample extends OptionBuyerLiftSample { regimeInput: MarketRegimeInput; regimeObservedAt:string; }
...
const exactIso=(s:string)=>{const t=Date.parse(s);return Number.isFinite(t)?new Date(t).toISOString():null;};
...
export function validateOptionBuyerRegimeLift(samples:OptionBuyerRegimeLiftSample[]):OptionBuyerRegimeLiftResult {
 ...
 for(const s of samples){
  ...
  const regimeTs=exactIso(s.regimeObservedAt),signalTs=exactIso(s.mdiInput.current.ts);if(regimeTs==null||signalTs==null||regimeTs!==signalTs){blockers.push("REGIME_TIMESTAMP_NOT_BOUND_TO_MDI_SIGNAL_TIMESTAMP");continue;}
