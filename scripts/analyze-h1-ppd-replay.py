...
def main():
    ...
    # Exact key only: timestamp + expiry + strike + CE/PE. No cross-expiry subtraction.
    ...
    for r in rows:
        ...
        # Prefer rows that are explicitly research eligible if duplicates somehow exist.
    ...
    # Endpoint selection: ATM row at endpoint, then compare same expiry+same strike exactly t-w.
    endpoints = []
    for (ts, expiry, strike), (ce, pe) in pairs.items():
        ce_off = ce.get("atm_offset")
        pe_off = pe.get("atm_offset")
        try:
            is_atm = float(ce_off) == 0 or float(pe_off) == 0
        except (TypeError, ValueError):
            is_atm = False
        if is_atm:
    ...
    for ts, expiry, strike, ce_to, pe_to in endpoints:
        ...
        for w in WINDOWS:
            ...
            ppd = ce_ret - pe_ret
            ctrl = "CE" if ppd > 0 else "PE" if ppd < 0 else None
            results[w].append({
                ...
                "rawPpdPp": ppd, "controllingSide": ctrl,
    ...
    for w, rr in results.items():
        ...
        summaries[str(w)] = {
            "windowMinutes": w, "sampleCount": len(rr),
            "controlCounts": dict(ctrl), "pairStateCounts": dict(states),
            "topAbsolutePpd": top,
        }

    # Multi-DTE sign alignment per exact endpoint timestamp/window.
    multi = {}
    for w, rr in results.items():
        ...
        multi[str(w)] = {
            "alignedCount": len(aligned), "conflictCount": len(conflicted),
            "firstAligned": aligned[:10], "firstConflicts": conflicted[:10],
        }

    # Compact 15-minute-clock timeline: retain available records nearest exact quarter-hour, within 2 minutes.
    timeline = []
    ...
    out = {
        "ok": True,
        "mode": "READ_ONLY_H1_PPD_REPLAY_AUDIT_V1",
        "productionImpact": "NONE",
        "request": data.get("request"),
        "sourceCounts": data.get("counts"),
        "sourceContinuity": data.get("continuity"),
        "method": {
            "windowsMinutes": list(WINDOWS),
            "identity": "EXACT_TIMESTAMP_SAME_EXPIRY_SAME_STRIKE_CE_PE",
            "endpoint": "ATM_OFFSET_ZERO",
            "price": "LTP_REPLAY",
            "formula": "rawPpdPp = CE_return_pct - PE_return_pct",
            "crossExpiryPremiumSubtraction": False,
            "nearestTimestampSubstitution": False,
        },
        "pairCount": len(pairs), "atmEndpointCount": len(endpoints),
        "expiriesObserved": sorted(expiries),
        "summaries": summaries, "multiDte": multi, "timeline15m": timeline,
    ...
    print(json.dumps({
        "ok": True, "mode": out["mode"], "pairCount": out["pairCount"], "atmEndpointCount": out["atmEndpointCount"],
        "summaries": {w: {"sampleCount": s["sampleCount"], "controlCounts": s["controlCounts"], "pairStateCounts": s["pairStateCounts"], "topAbsolutePpd": s["topAbsolutePpd"][:5]} for w, s in summaries.items()},
        "multiDte": multi,
    }, separators=(",", ":")))
