...
<body>
  ...
  <script>
    ...
    function renderOptionsChain(symbol, m) {
      ...
      html += '</div>';
      html += '<div class="timestamp">Showing ' + (symbol === 'BANKNIFTY' ? 'ATM \u00b16 (13 strikes)' : 'ATM \u00b110 (21 strikes)') + ' — matches the spec\u2019s detailed-table range. ATM \u00b120 background range is not fetched. Max OI Addition and strongest writing/buying/covering/unwinding strikes are not yet tracked — DATA UNAVAILABLE for those.</div>';
      return html;
    }
    ...
    function renderContextMacroVix() {
      ...
      html += '</div>';
      html += '<div class="timestamp">This is MCX Crude Oil futures (roughly WTI-linked), not the ICE Brent contract the spec names — Kite does not offer Brent directly. Treat as a risk-modifier, not a direct CE/PE trigger, per spec.</div>';
      return html;
    }
