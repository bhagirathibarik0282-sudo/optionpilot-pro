...
<body>
  ...
  <script>
    ...
    function renderDataReliabilityCard() {
      ...
      html += '<div class="timestamp">Sync tolerance: ' + (SYNC_TOLERANCE_MS / 1000) + 's (PROVISIONAL, not backtested). This checks timestamp sync ACROSS NIFTY/BANKNIFTY/SENSEX \u2014 separate spot-vs-futures-vs-options timestamps within a single index are not tracked yet, so that finer-grained field is hidden rather than guessed. No CE/PE decision or order-review button exists in this dashboard yet (by design \u2014 no automated order placement), so there is nothing to disable for that rule right now.</div>';
      html += '</div>';
      return html;
    }
    ...
    function computeCardStatus(m) {
      ...
      if (!data) return 'WAITING FOR LIVE SESSION';
    ...
    function cardStatusColor(status) {
      ...
      if (status === 'PREVIOUS SESSION CONTEXT' || status === 'WAITING FOR LIVE SESSION') return 'var(--muted)';
