...
async function fetchIndexData(
  ...
  try {
    ...
    baseMetrics.exchangeTimestamp = spotQuote.last_trade_time || spotQuote.timestamp || null;
...
<body>
  ...
  <script>
    ...
    function cardStatusColor(status) {
      ...
      if (status === 'PREVIOUS SESSION CONTEXT' || status === 'WAITING FOR LIVE SESSION' || status.indexOf('PRE-MARKET') === 0) return 'var(--muted)';
    ...
    function renderPremiumPairCard(symbol, m) {
      ...
      html += rowLine('Expiry', exp.expiry);
      html += rowLine('ATM Strike', atmStrikeVal != null ? String(atmStrikeVal) : 'DATA UNAVAILABLE');
      html += rowLine('Previous ATM', continuity.previousAtm != null ? continuity.previousAtm + (continuity.switched ? ' (changed this refresh — continuity check only, not a signal)' : ' (unchanged)') : 'DATA UNAVAILABLE');

      if (atmCe) {
        const ceKey = symbol + '_' + exp.expiry + '_CE_' + atmCe.strike;
        recordStrikeOi(ceKey, atmCe.oi);
        const ce3m = computeOiChangeFromSnapshot(ceKey, 3);
        const ce15m = computeOiChangeFromSnapshot(ceKey, 15);
        ...
        const ceSpread = computeSpreadPct(atmCe.bid, atmCe.ask);

        html += '<div class="fii-section-label">CE ' + atmCe.strike + '</div>';
        html += rowLine('CE LTP', atmCe.lastPrice.toFixed(2));
        html += rowLine('CE Total OI', atmCe.oi != null ? atmCe.oi.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('CE Change in OI', ceOiInfo.delta != null ? (ceOiInfo.delta >= 0 ? '+' : '') + ceOiInfo.delta.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('CE 3-min OI Change', ce3m != null ? (ce3m >= 0 ? '+' : '') + ce3m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('CE 15-min OI Change', ce15m != null ? (ce15m >= 0 ? '+' : '') + ce15m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('CE Volume', atmCe.volume != null ? atmCe.volume.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('CE Bid / Ask', (atmCe.bid > 0 && atmCe.ask > 0) ? atmCe.bid.toFixed(2) + ' / ' + atmCe.ask.toFixed(2) : 'DATA UNAVAILABLE');
        html += rowLine('CE Spread %', ceSpread != null ? ceSpread.toFixed(2) + '%' : 'DATA UNAVAILABLE');
        html += rowLine('CE Liquidity', (atmCe.bid > 0) ? (isLiquid(atmCe) ? 'Liquid' : 'Not Liquid') : 'DATA UNAVAILABLE');
        html += rowLine('CE VWAP', atmCe.vwapSource === 'UNVERIFIED AVERAGE PRICE — NOT VWAP' ? 'UNVERIFIED AVERAGE PRICE — NOT VWAP (' + atmCe.vwap.toFixed(2) + ')' : 'VWAP UNAVAILABLE');
        html += rowLine('CE Interpretation', ceInterp);
      }

      if (atmPe) {
        const peKey = symbol + '_' + exp.expiry + '_PE_' + atmPe.strike;
        recordStrikeOi(peKey, atmPe.oi);
        const pe3m = computeOiChangeFromSnapshot(peKey, 3);
        const pe15m = computeOiChangeFromSnapshot(peKey, 15);
        ...
        const peSpread = computeSpreadPct(atmPe.bid, atmPe.ask);

        html += '<div class="fii-section-label">PE ' + atmPe.strike + '</div>';
        html += rowLine('PE LTP', atmPe.lastPrice.toFixed(2));
        html += rowLine('PE Total OI', atmPe.oi != null ? atmPe.oi.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('PE Change in OI', peOiInfo.delta != null ? (peOiInfo.delta >= 0 ? '+' : '') + peOiInfo.delta.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('PE 3-min OI Change', pe3m != null ? (pe3m >= 0 ? '+' : '') + pe3m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('PE 15-min OI Change', pe15m != null ? (pe15m >= 0 ? '+' : '') + pe15m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('PE Volume', atmPe.volume != null ? atmPe.volume.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        html += rowLine('PE Bid / Ask', (atmPe.bid > 0 && atmPe.ask > 0) ? atmPe.bid.toFixed(2) + ' / ' + atmPe.ask.toFixed(2) : 'DATA UNAVAILABLE');
        html += rowLine('PE Spread %', peSpread != null ? peSpread.toFixed(2) + '%' : 'DATA UNAVAILABLE');
        html += rowLine('PE Liquidity', (atmPe.bid > 0) ? (isLiquid(atmPe) ? 'Liquid' : 'Not Liquid') : 'DATA UNAVAILABLE');
        html += rowLine('PE VWAP', atmPe.vwapSource === 'UNVERIFIED AVERAGE PRICE — NOT VWAP' ? 'UNVERIFIED AVERAGE PRICE — NOT VWAP (' + atmPe.vwap.toFixed(2) + ')' : 'VWAP UNAVAILABLE');
        html += rowLine('PE Interpretation', peInterp);
      }
