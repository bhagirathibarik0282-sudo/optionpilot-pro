...
<body>
  ...
  <script>
    ...
    function renderPremiumPairCard(symbol, m) {
      ...
      const marketOpen = isMarketOpenNow();
      // Rule 6: this card gets its own more specific status text when the
      // market is closed — the shared computeCardStatus() text is more
      // generic and is left unchanged for other cards.
      ...
      function renderLeg(label, leg, key) {
        // Rule 1/2/3: OI-change fields only computed from real snapshots,
        // and only while the market is live — never a derived/stale "0"
        // during PREVIOUS SESSION CONTEXT.
        ...
        block += rowLine(label + ' Change in OI', (marketOpen && changeInOi != null) ? (changeInOi >= 0 ? '+' : '') + changeInOi.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        block += rowLine(label + ' 3-min OI Change', (marketOpen && change3m != null) ? (change3m >= 0 ? '+' : '') + change3m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        block += rowLine(label + ' 15-min OI Change', (marketOpen && change15m != null) ? (change15m >= 0 ? '+' : '') + change15m.toLocaleString('en-IN') + ' qty' : 'DATA UNAVAILABLE');
        ...
        block += rowLine(label + ' Spread %', spread != null ? spread.toFixed(2) + '%' : 'DATA UNAVAILABLE');
        block += rowLine(label + ' Liquidity', leg.bid > 0 ? (isLiquid(leg) ? 'Liquid' : 'Not Liquid') : 'DATA UNAVAILABLE');
        ...
        block += rowLine(label + ' Interpretation', marketOpen ? interp : 'DATA UNAVAILABLE');
        block += '</div>';
        return block;
      }
      ...
      html += partialDataFooter();
      html += '<div class="timestamp">Interpretation labels describe price+OI behaviour patterns only \u2014 not proof of who initiated any trade. OI-change fields (Change in OI, 3-min, 15-min) are only computed while the market is live and a real earlier snapshot exists \u2014 hidden entirely outside market hours rather than showing a stale or zero value. CE/PE VWAP shown here is Kite\u2019s raw average_price, explicitly UNVERIFIED against provider documentation as a true session VWAP \u2014 treat as an unconfirmed reference number only. Total OI and Change in OI are two separate fields, never merged. Display-and-logging only \u2014 does not affect the Verdict or Alignment cards, and there is no order-placement feature to enable.</div>';
      html += '</div>';
      return html;
    }
