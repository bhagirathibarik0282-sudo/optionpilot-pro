...
<body>
  ...
  <script>
    ...
    function renderPremiumPairCard(symbol, m) {
      ...
      html += partialDataFooter();
      html += '<div class="timestamp">Interpretation labels describe price+OI behaviour patterns only \u2014 not proof of who initiated any trade. OI-change fields (Change in OI, 3-min, 15-min) are only computed once per real data fetch (not re-triggered by switching tabs) and only while the market is live \u2014 hidden entirely outside market hours or before a real earlier snapshot exists, rather than showing a stale or zero value. CE/PE VWAP shown here is Kite\u2019s raw average_price, explicitly UNVERIFIED against provider documentation as a true session VWAP \u2014 treat as an unconfirmed reference number only. Total OI and Change in OI are two separate fields, never merged. Display-and-logging only \u2014 does not affect the Verdict or Alignment cards, and there is no order-placement feature to enable.</div>';
      html += '</div>';
      return html;
    }
