...
<body>
  ...
  <script>
    ...
    function statusSpotStructure(symbol, m) {
      ...
      const vwapState = computeSpotVwapState(m);
      if (vwapState === 'DATA UNAVAILABLE') return mkStatus('PARTIAL', 'NONE', 'DISPLAY_ONLY', true, base.data_age_seconds, base.quote_age_seconds, null, m.snapshotId, ['SPOT_VWAP_UNAVAILABLE']);
      ...
      const direction = vwapState === 'ABOVE' ? 'BULLISH' : (vwapState === 'BELOW' ? 'BEARISH' : 'NEUTRAL');
