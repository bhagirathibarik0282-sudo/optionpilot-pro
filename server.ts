...
<body>
  ...
  <script>
    ...
    function computeMissingReasons(m) {
      ...
      if (!m.vwap || m.vwap === 0) reasons.push('Spot VWAP not confirmed');
    ...
    function computeSpotVwapState(m) {
      if (!m || m.error || !m.vwap || m.vwap <= 0) return 'DATA UNAVAILABLE';
      if (m.current > m.vwap) return 'ABOVE';
      if (m.current < m.vwap) return 'BELOW';
      return 'AT VWAP';
    }
    ...
    function renderSpotFuturesStatusRow(symbol, m) {
      ...
      const spotVwapState = computeSpotVwapState(m);
      ...
      const spotBias = spotVwapState === 'ABOVE' ? 'bullish' : spotVwapState === 'BELOW' ? 'bearish' : null;
    ...
    function renderSpotFuturesCoreStatus() {
      ...
      html += '<div class="timestamp">"Spot VWAP" reuses the same futures-derived VWAP proxy used throughout this dashboard \u2014 no separately computed spot-only VWAP exists. 5m/15m structure is a simplified monotonic-trend read on the spot samples collected this session so far (not true OHLC swing-high/swing-low structure) \u2014 shows UNCONFIRMED until at least 3 samples exist in that window.</div>';
      return html;
    }

    function renderVerdict() {
      let html = renderDataReliabilityCard();
      if (!data) {
        if (isPreMarket()) {
    ...
    function renderTrackerReadiness(symbol, m) {
      ...
      html += '<div class="fii-section-label">Delta</div>';
      if (atmCe) html += rowLine('CE Delta Source', classifyDeltaSource(atmCe));
      if (atmPe) html += rowLine('PE Delta Source', classifyDeltaSource(atmPe));
      html += rowLine('Selection Method', DELTA_SELECTION_METHOD);

      html += '<div class="fii-section-label">ATM Shift Tracker</div>';
      html += rowLine('Current ATM', shiftInfo.current != null ? String(shiftInfo.current) : 'DATA UNAVAILABLE');
      html += rowLine('Previous ATM', shiftInfo.previous != null ? String(shiftInfo.previous) : 'DATA UNAVAILABLE');
      html += rowLine('Shift Timestamp', shiftInfo.lastShiftTime ? shiftInfo.lastShiftTime.toLocaleTimeString() : 'DATA UNAVAILABLE');
      html += rowLine('Last Shift Direction', shiftInfo.shiftCount > 0 ? shiftInfo.lastShiftDirection : 'UNCHANGED');
      html += rowLine('Shifts This Session', String(shiftInfo.shiftCount));
      html += rowLine('Spot at Last Shift', shiftInfo.spotAtShift != null ? shiftInfo.spotAtShift.toFixed(2) : 'DATA UNAVAILABLE');
      ...
      html += legReadiness('CE', atmCe);
      html += legReadiness('PE', atmPe);

      html += '<div class="fii-section-label">Synchronization Tracker</div>';
      html += rowLine('Exchange Timestamp', m.exchangeTimestamp ? new Date(m.exchangeTimestamp).toLocaleTimeString() : 'DATA UNAVAILABLE');
      html += rowLine('Backend Received Timestamp', m.timestamp ? new Date(m.timestamp).toLocaleTimeString() : 'DATA UNAVAILABLE');
      html += rowLine('Snapshot ID', m.snapshotId ? '\u2026' + m.snapshotId.slice(-12) : 'DATA UNAVAILABLE');
      ...
      html += '<div class="fii-section-label">Readiness Result</div>';
      html += '<div style="color:' + (ready ? 'var(--green)' : 'var(--gold)') + '; font-weight:700; font-size:0.85rem; margin-top:4px;">' + (ready ? 'TRACKERS READY FOR STEP 5B' : 'STEP 5B BLOCKED \u2014 MISSING: ' + missing.join(', ')) + '</div>';
    ...
    function renderAlignmentTab(symbol, m) {
      let html = renderTrackerReadiness(symbol, m);
    ...
    function computeStep5BConclusion(symbol, m) {
      ...
      const ceStrongOrSupportive = currentCeState.indexOf('STRONG') !== -1 || currentCeState.indexOf('SUPPORTIVE') !== -1;
      const peStrongOrSupportive = currentPeState.indexOf('STRONG') !== -1 || currentPeState.indexOf('SUPPORTIVE') !== -1;
      const peWeak = currentPeState.indexOf('WEAK') !== -1 || currentPeState.indexOf('NEUTRAL') !== -1;
      const ceWeak = currentCeState.indexOf('WEAK') !== -1 || currentCeState.indexOf('NEUTRAL') !== -1;

      let crossExpiryStatus = 'UNCONFIRMED';
      if (ceStrongOrSupportive && peWeak && laterCeSupport && !laterCeConflict) {
        crossExpiryStatus = (currentCeState.indexOf('STRONG') !== -1 && laterCe[0] && laterCe[0].state.indexOf('STRONG') !== -1)
          ? 'STRONG CROSS-EXPIRY ITM CE ALIGNMENT' : 'CROSS-EXPIRY ITM CE ALIGNMENT';
      } else if (peStrongOrSupportive && ceWeak && laterPeSupport && !laterPeConflict) {
        crossExpiryStatus = (currentPeState.indexOf('STRONG') !== -1 && laterPe[0] && laterPe[0].state.indexOf('STRONG') !== -1)
          ? 'STRONG CROSS-EXPIRY ITM PE ALIGNMENT' : 'CROSS-EXPIRY ITM PE ALIGNMENT';
      } else if ((ceStrongOrSupportive && laterCeConflict) || (peStrongOrSupportive && laterPeConflict)) {
        crossExpiryStatus = 'LONGER-EXPIRY CONFLICT';
      } else if ((ceStrongOrSupportive && !laterCeSupport) || (peStrongOrSupportive && !laterPeSupport)) {
        crossExpiryStatus = 'CURRENT-EXPIRY-ONLY MOVE \u2014 POSSIBLE EXPIRY NOISE';
      } else if (currentCeState.indexOf('STRONG') !== -1 && currentPeState.indexOf('STRONG') !== -1) {
        crossExpiryStatus = 'BOTH SIDES EXPANDING \u2014 VOLATILITY CAUSE UNVERIFIED';
      } else if (currentCeState.indexOf('WEAK') !== -1 && currentPeState.indexOf('WEAK') !== -1) {
        crossExpiryStatus = 'BOTH SIDES WEAK \u2014 RANGE / THETA';
      }

      // Current-expiry trade conclusion (L)
      let tradeConclusion;
      if (crossExpiryStatus.indexOf('CE ALIGNMENT') !== -1) {
        tradeConclusion = (ceResp === 'RESPONSIVE') ? 'CURRENT ATM CE SUPPORTIVE' : (ceItmResp === 'RESPONSIVE' ? 'CURRENT 1-ITM CE PREFERRED' : 'CURRENT EXPIRY CONTRACT UNSUITABLE');
      } else if (crossExpiryStatus.indexOf('PE ALIGNMENT') !== -1) {
        tradeConclusion = (peResp === 'RESPONSIVE') ? 'CURRENT ATM PE SUPPORTIVE' : (peItmResp === 'RESPONSIVE' ? 'CURRENT 1-ITM PE PREFERRED' : 'CURRENT EXPIRY CONTRACT UNSUITABLE');
      } else if (crossExpiryStatus === 'LONGER-EXPIRY CONFLICT') {
        tradeConclusion = 'WAIT \u2014 LONGER-EXPIRY CONFLICT';
      } else if (ceResp === 'NO RESPONSE' && peResp === 'NO RESPONSE') {
        tradeConclusion = 'CURRENT EXPIRY CONTRACT UNSUITABLE';
      } else {
        tradeConclusion = 'WAIT \u2014 DATA INCOMPLETE';
      }

      const itmDirection = crossExpiryStatus.indexOf('CE') !== -1 ? 'CE' : (crossExpiryStatus.indexOf('PE') !== -1 ? 'PE' : null);
      const contract = (m.futuresContracts && m.futuresContracts[0]) || null;
      ...
      let finalStatus = crossExpiryStatus;
      if (threeIndexContext === 'THREE-INDEX CONFLICT' && (crossExpiryStatus.indexOf('ALIGNMENT') !== -1)) {
        finalStatus = 'ALIGNMENT WATCH \u2014 CROSS-INDEX CONFLICT';
      }
      ...
      return {
        ...
        crossExpiryStatus: crossExpiryStatus, tradeConclusion: tradeConclusion,
    ...
    function renderStep5BCard(symbol, m) {
      ...
      const statusColor = result.finalStatus.indexOf('CE ALIGNMENT') !== -1 ? 'var(--green)' : result.finalStatus.indexOf('PE ALIGNMENT') !== -1 ? 'var(--red)' : result.finalStatus.indexOf('CONFLICT') !== -1 ? 'var(--gold)' : 'var(--muted)';
      html += '<div style="margin-top:10px; padding-top:10px; border-top:1px solid var(--border);">';
      html += '<div style="color:var(--muted); font-size:0.65rem; text-transform:uppercase;">Final</div>';
      html += '<div style="color:' + statusColor + '; font-weight:700; font-size:0.85rem;">' + escapeHtml(result.finalStatus) + '</div>';
      html += '<div style="color:var(--text); font-size:0.78rem; margin-top:2px;">' + escapeHtml(result.tradeConclusion) + '</div>';
      html += '</div>';
