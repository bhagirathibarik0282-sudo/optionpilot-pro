...
<body>
  ...
  <script>
    ...
    function renderDataReliabilityCard() {
      ...
      if (r.lockReason) {
        html += '<div style="background:rgba(229,72,77,0.14); border:2px solid var(--red); border-radius:6px; padding:8px; margin:6px 0; color:var(--red); font-weight:700; font-size:0.8rem;">\u26a0 ' + r.lockReason + '</div>';
      }
      ...
      if (r.perIndex) {
        ...
        html += rowLine('Snapshot ID', r.snapshotId || 'DATA UNAVAILABLE');
      } else {
      ...
      html += '<div class="timestamp">Sync tolerance: ' + (SYNC_TOLERANCE_MS / 1000) + 's (PROVISIONAL, not backtested). This checks timestamp sync ACROSS NIFTY/BANKNIFTY/SENSEX \u2014 separate spot-vs-futures-vs-options timestamps within a single index are not tracked yet, so DATA UNAVAILABLE is shown rather than guessing at that finer level. No CE/PE decision or order-review button exists in this dashboard yet (by design \u2014 no automated order placement), so there is nothing to disable for that rule right now.</div>';
      html += '</div>';
      return html;
    }
    ...
    function renderSpotFuturesStatusRow(symbol, m) {
      if (!m || m.error) {
        return '<div class="premium-card" style="margin-bottom:10px;"><div class="card-title">' + symbol + ' \u2014 Spot + Futures Status</div><div class="unavailable-text">DATA UNAVAILABLE</div></div>';
      }
      ...
      const ageSec = m.timestamp ? Math.round((Date.now() - new Date(m.timestamp).getTime()) / 1000) : null;
      const ageLabel = marketOpen
        ? (ageSec != null ? 'LIVE \u2014 AGE ' + ageSec + 's' : 'DATA UNAVAILABLE')
      ...
      html += rowLine('Data Age', ageLabel);
      html += '</div>';
      return html;
    }
    ...
    function renderVerdict() {
      ...
      if (!data) return html + '<div class="loading">Loading verdict...</div>';
    ...
    function renderOptionsPremium(symbol, m) {
      ...
      if (atmCe) {
        ...
        html += rowLine('Delta', atmCe.delta ? atmCe.delta.toFixed(3) : '—');
      }
      if (atmPe) {
        ...
        html += rowLine('Delta', atmPe.delta ? atmPe.delta.toFixed(3) : '—');
      }
      html += '</div>';
      html += '<div class="timestamp">Delta/Vega/Theta are Black-Scholes estimates (spot, strike, IV, days-to-expiry) — Kite does not publish Greeks directly. 5m/15m/30m/1h premium momentum and liquidity status are not yet tracked — DATA UNAVAILABLE rather than guessed.</div>';
      return html;
    }
