...
async function captureRecorderSnapshot(reason: string): Promise<void> {
  try {
    ...
    const entry: RecorderSnapshot = {
      ...
      reason,
      NIFTY: toRecorderIndexSnapshot(snapshot.NIFTY),
      BANKNIFTY: toRecorderIndexSnapshot(snapshot.BANKNIFTY),
      SENSEX: toRecorderIndexSnapshot(snapshot.SENSEX),
...
<body>
  ...
  <script>
    ...
    function renderConnectionStatusBar() {
      ...
      html += '<span style="color:' + color + '; font-weight:700;">' + escapeHtml(state) + '</span>';
      html += '<span style="color:var(--muted);">Updated: ' + escapeHtml(lastUpdateText) + '</span>';
    ...
    function computeMissingReasons(m) {
      ...
      if (!m.vwap || m.vwap === 0) reasons.push('Futures VWAP not confirmed');
    ...
    function renderSpotFuturesStatusRow(symbol, m) {
      ...
      html += rowLine('Futures LTP vs Futures VWAP', futVwapState);
    ...
    function renderCompactIndexCard(symbol, m, isSensex) {
      ...
      if (!m || m.error) {
        html += '<div class="unavailable-text">DATA UNAVAILABLE</div></div>';
        return html;
      }
    ...
    function renderRecorderMiniBar() {
      ...
      return '<div style="display:flex; justify-content:space-between; padding:6px 12px; background:rgba(0,0,0,0.15); border-radius:8px; margin-bottom:8px; font-size:0.68rem;"><span style="color:' + color + '; font-weight:700;">Recorder ' + (s.status === 'RECORDING' ? 'ON' : s.status) + '</span><span style="color:var(--muted);">' + s.snapshotCount + ' snapshots</span></div>';
    }
    ...
    async function initialize() {
      ...
      await Promise.all([loadNews(), loadHolidays(), loadFiiDii(), loadRecorderStatus()]);
      if (kiteConnected) {
        await Promise.all([fetchData(), loadCommodities()]);
      } else {
...
app.get("/api/recorder/session.csv", (c) => {
  const rows = [
    "snapshot_id,backend_timestamp,reason,symbol,spot,change,pdh,pdl,vwap,futures_ltp,futures_oi,atm_strike,ce_ltp,pe_ltp,ce_oi,pe_oi,exchange_timestamp,snapshot_sync_id,fii_cash_cr,dii_cash_cr",
  ];
  const esc = (v: unknown) => {
    if (v == null) return "";
    const s = String(v);
  ...
  for (const snap of recorderSession.snapshots) {
    (["NIFTY", "BANKNIFTY", "SENSEX"] as const).forEach((sym) => {
      ...
      rows.push(
        [
          snap.snapshotId, snap.backendTimestamp, snap.reason, sym,
