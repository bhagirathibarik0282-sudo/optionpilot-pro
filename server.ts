...
function computeTruthReport(m: IndexMetrics | undefined): TruthReport {
  ...
  fields.spot = classifyTruthField(m.exchangeTimestamp, TRUTH_THRESHOLDS_MS.spot);
...
<body>
  ...
  <script>
    ...
    async function initialize() {
      ...
      await Promise.all([loadNews(), loadHolidays(), loadFiiDii(), loadRecorderStatus(), loadDriveStatus(), loadJournalData(), loadTruthStatus(), loadHealthStatus(), loadRecoveryStatus()]);
      await loadDnaStatus();
      if (kiteConnected) {
        await Promise.all([fetchData(), loadCommodities()]);
      } else {
...
app.get("/api/drive/replay/:date", async (c) => {
  ...
  if (!record) {
    return c.json({ error: "No verified archive found in this server's in-memory index for that date. It may still exist in Google Drive itself, but is not currently indexed \u2014 see Module 3's known Phase-1 limitation." }, 404);
  }
  const rawFileId = record.fileIds.raw;
  if (!rawFileId) return c.json({ error: "Archive record has no Raw JSON file reference." }, 404);
  ...
  if (!fileRes.ok) return c.json({ error: "Could not download archived Raw JSON from Drive (HTTP " + fileRes.status + ")" }, 502);
