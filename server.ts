...
// Aligns to a fixed 30-minute grid anchored at market open (9:15 IST) — so
// checkpoints land on 9:15, 9:45, 10:15, ... instead of whatever moment a
// refresh happened to fire.
function getGridSlot(): string {
  const indiaNow = new Date(Date.now() + INDIA_OFFSET_MS);
  const hh = indiaNow.getUTCHours();
  const mm = indiaNow.getUTCMinutes();
  const totalMin = hh * 60 + mm;
  const anchor = 9 * 60 + 15; // 9:15 IST
  if (totalMin < anchor) return indiaTimeHHMM();
  const slot = anchor + Math.floor((totalMin - anchor) / 30) * 30;
  const slotHH = Math.floor(slot / 60);
  const slotMM = slot % 60;
  return String(slotHH).padStart(2, "0") + ":" + String(slotMM).padStart(2, "0");
}
...
function recordDailyJournal(snapshot: Record<string, IndexMetrics>) {
  ...
  if (!lastSnapshot || lastSnapshot.time !== gridSlot) {
    entry.snapshots.push({ time: gridSlot, summaries });
    if (entry.snapshots.length > 20) entry.snapshots.shift();
  } else {
    // Still inside the same 30-minute window — keep the latest data live
    // without adding a new grid row.
    entry.snapshots[entry.snapshots.length - 1] = { time: gridSlot, summaries };
  }
...
<body>
  <div class="container">
    ...
    <div class="tabs">
      <button class="tab-btn" onclick="switchTab('DAILYSUMMARY')">⭐ Summary</button>
    ...
    <div id="DAILYSUMMARY" class="tab-content"></div>
  ...
  <script>
    ...
    // VERDICT tab — per the new mobile spec. Combines data already computed
    // elsewhere (signal, futuresVwapBias, Gap Score) into per-index bias +
    // one overall combined verdict. Weighting is NIFTY 40% / BANKNIFTY 30% /
    // SENSEX 30% — deliberately not an equal 33/33/33 split (spec rule),
    // clearly documented rather than hidden.
    ...
    function renderContext() {
      return '<div class="loading">CONTEXT tab — coming in a future step (PREVIOUS / FII-DII / GAP CHECK / MACRO-VIX / SIGNAL REVIEW).</div>';
    }
    ...
    // at-a-glance "are premiums broadly rising or falling" signal. Kept
    // after Swing Tracker was removed, now shown on the Summary tab instead.
    function renderPremiumBreadthCard() {
      if (!data) return '';
    ...
    async function refreshData() {
      ...
      await Promise.all([fetchData(), loadAlignment(), loadHeatmap(), loadCommodities(), loadDailySummary()]);
    ...
    async function initialize() {
      ...
      if (kiteConnected) {
        ...
        ['NIFTY', 'BANKNIFTY', 'SENSEX', 'HEATMAP', 'COMMODITIES', 'VIXCORR', 'ALIGNMENT', 'DAILYSUMMARY', 'VERDICT', 'CONTEXT']
          .forEach((id) => {
