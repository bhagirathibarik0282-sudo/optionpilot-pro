...
<body>
  ...
  <script>
    ...
    function renderOverviewTab(symbol, m) {
      ...
      html += rowLine('Gap %', 'DATA UNAVAILABLE');
      html += rowLine('Day Open / High / Low', 'DATA UNAVAILABLE');
      html += rowLine('First 15m High / Low', 'DATA UNAVAILABLE');
      html += rowLine('15m / 30m / 1h Trend', 'DATA UNAVAILABLE');
      html += '</div>';
      ...
      html += '<div class="timestamp">Fields not yet tracked (day open/high/low, first-15m range, 15m/30m/1h trend) show DATA UNAVAILABLE rather than a guessed value, per spec rule.</div>';
      return html;
    }
    ...
    function renderIndexPage(symbol) {
      ...
      else if (tab === 'FUTURES') html += '<div class="loading">FUTURES tab — coming in a future step (near/next/far month contracts).</div>';
      else if (tab === 'OPTIONS') html += '<div class="loading">OPTIONS tab — coming in a future step (Premium/Chain/Expiry/Walls & PCR).</div>';
      else html += '<div class="loading">ALIGNMENT tab — coming in a future step (constituent contribution).</div>';
