...
// ============== GLOBAL SENTIMENT MANUAL ENTRY (Kite has no international data) ==============
// Zerodha Kite only covers NSE/BSE/MCX — it does not provide Dow Jones,
// Nasdaq, SGX/GIFT Nifty, or other overseas index data. This is filled in
// by hand, same in-memory-only caveat as FII/DII.
interface GlobalSentimentEntry {
  date: string; // YYYY-MM-DD
  giftNifty: "Bullish" | "Bearish" | "Flat";
  usMarkets: "Bullish" | "Bearish" | "Flat";
  asianMarkets: "Bullish" | "Bearish" | "Flat";
  note: string;
  createdAt: string;
}

const globalSentimentEntries: GlobalSentimentEntry[] = [];
const GLOBAL_SENTIMENT_MAX_ENTRIES = 30;
...
<body>
  ...
  <script>
    ...
    async function loadGlobalSentiment() {
      try {
        const response = await fetch('/api/global-sentiment');
        const json = await response.json();
        globalSentimentData = response.ok ? json : { error: json.error || 'Failed to load global sentiment' };
        updateUI();
      } catch (err) {
        console.error('Failed to load global sentiment:', err);
        globalSentimentData = { error: err.message };
      }
    }

    async function saveGlobalSentiment() {
      const getVal = (id) => document.getElementById(id).value;
      const date = getVal('gsDate') || new Date().toISOString().slice(0, 10);
      const giftNifty = getVal('gsGiftNifty');
      const usMarkets = getVal('gsUsMarkets');
      const asianMarkets = getVal('gsAsianMarkets');
      const note = getVal('gsNote');

      try {
        const response = await fetch('/api/global-sentiment', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ date, giftNifty, usMarkets, asianMarkets, note }),
        });
        if (!response.ok) throw new Error('Save failed');
        showSuccess('✓ Global sentiment saved for ' + date);
        await loadGlobalSentiment();
      } catch (err) {
        showError('Could not save global sentiment: ' + err.message);
      }
    }
    ...
    function getDayBias(entry, sym) {
      if (!entry.snapshots || entry.snapshots.length === 0) return { label: 'N/A', color: 'var(--muted)' };
      const last = entry.snapshots[entry.snapshots.length - 1];
      const summary = last.summaries && last.summaries[sym];
      if (!summary) return { label: 'N/A', color: 'var(--muted)' };
      const item = summary.checklist.find((c) => c.label === 'Signal Confirmed');
      if (item && item.pass) {
        return item.color === 'bullish' ? { label: 'Bullish', color: 'var(--green)' } : { label: 'Bearish', color: 'var(--red)' };
      }
      return { label: 'Neutral', color: 'var(--muted)' };
    }

    function renderLast5DaysTrend(entries) {
      const last5 = entries.slice(-5);
      if (last5.length === 0) return '';
      let html = '<div class="premium-card" style="margin-bottom:16px;">';
      html += '<div class="card-title">Last 5 Days — Market Trend</div>';
      html += '<div class="table-scroll"><table style="width:100%; font-family: var(--font-mono); font-size:0.75rem; border-collapse:collapse;">';
      html += '<thead><tr style="color:var(--muted-dim);"><th style="text-align:left; padding:4px;">Date</th><th style="padding:4px;">NIFTY</th><th style="padding:4px;">BANKNIFTY</th><th style="padding:4px;">SENSEX</th></tr></thead><tbody>';
      last5.slice().reverse().forEach((e) => {
        html += '<tr style="border-top:1px solid var(--border);">';
        html += '<td style="padding:4px; color:var(--text);">' + e.date + '</td>';
        ['NIFTY', 'BANKNIFTY', 'SENSEX'].forEach((sym) => {
          const b = getDayBias(e, sym);
          html += '<td style="padding:4px; text-align:center; color:' + b.color + '; font-weight:700;">' + b.label + '</td>';
        });
        html += '</tr>';
      });
    ...
    function renderGlobalSentimentForm() {
      const today = new Date().toISOString().slice(0, 10);
      let html = '<div class="premium-card" style="margin-bottom:16px;">';
      html += '<div class="card-title">Global Sentiment (manual — Kite has no international data)</div>';
      html += '<div class="fii-form-grid">';
      html += '<div class="fii-field"><label>Date</label><input type="date" id="gsDate" value="' + today + '"></div>';
      html += '<div class="fii-field"><label>SGX/GIFT Nifty</label><select id="gsGiftNifty"><option>Flat</option><option>Bullish</option><option>Bearish</option></select></div>';
      html += '<div class="fii-field"><label>US Markets (prev close)</label><select id="gsUsMarkets"><option>Flat</option><option>Bullish</option><option>Bearish</option></select></div>';
      html += '<div class="fii-field"><label>Asian Markets</label><select id="gsAsianMarkets"><option>Flat</option><option>Bullish</option><option>Bearish</option></select></div>';
      html += '</div>';
      html += '<div class="fii-field" style="margin-top:10px;"><label>Note (optional)</label><input type="text" id="gsNote" placeholder="e.g. Fed rate decision awaited" style="width:100%;"></div>';
      html += '<button class="btn primary" style="margin-top:10px;" onclick="saveGlobalSentiment()">💾 Save</button>';
      html += '</div>';
      return html;
    }

    function renderGlobalSentiment() {
      let html = renderGlobalSentimentForm();
      if (!globalSentimentData) {
        html += '<div class="loading">Loading global sentiment...</div>';
        return html;
      }
      if (globalSentimentData.error) {
        html += '<div class="error">⚠️ ' + escapeHtml(globalSentimentData.error) + '</div>';
        return html;
      }
      const entries = globalSentimentData.entries || [];
      if (entries.length > 0) {
        const latest = entries[entries.length - 1];
        const biasColorOf = (v) => v === 'Bullish' ? 'bullish' : v === 'Bearish' ? 'bearish' : 'neutral';
        html += '<div class="premium-card" style="margin-bottom:16px;">';
        html += '<div class="card-title">Latest (' + latest.date + ')</div>';
        html += renderAlignRow('SGX/GIFT Nifty', '', latest.giftNifty, biasColorOf(latest.giftNifty));
        html += renderAlignRow('US Markets', '', latest.usMarkets, biasColorOf(latest.usMarkets));
        html += renderAlignRow('Asian Markets', '', latest.asianMarkets, biasColorOf(latest.asianMarkets));
        if (latest.note) html += '<div style="color:var(--muted); font-size:0.8rem; margin-top:6px;">' + escapeHtml(latest.note) + '</div>';
        html += '</div>';
      }
      html += '<div class="timestamp">Manually entered — Kite/Zerodha does not provide international market data. Stored in server memory only.</div>';
      return html;
    }

    function renderMasterChecklist() {
      if (!data || !data.NIFTY || data.NIFTY.error) return '';
      const items = [];

      const gs = data.NIFTY.gapScore;
      if (gs) items.push({ label: 'Gap Score', dir: gs.score > 10 ? 'bullish' : gs.score < -10 ? 'bearish' : 'neutral', detail: gs.verdict + ' (' + gs.score + ')' });

      if (alignmentData && alignmentData.overallBias) {
        items.push({ label: 'Market Alignment', dir: alignmentData.overallBias === 'BULLISH' ? 'bullish' : alignmentData.overallBias === 'BEARISH' ? 'bearish' : 'neutral', detail: alignmentData.overallBias });
      }

      if (data.NIFTY.pcr != null) {
        items.push({ label: 'PCR Tilt', dir: data.NIFTY.pcr > 1.1 ? 'bullish' : data.NIFTY.pcr < 0.85 ? 'bearish' : 'neutral', detail: data.NIFTY.pcr.toFixed(2) });
      }

      if (fiiDiiData && fiiDiiData.entries && fiiDiiData.entries.length > 0) {
        const last5 = fiiDiiData.entries.slice(-5);
        const pos = last5.filter((e) => e.fiiCashCr > 0).length;
        items.push({ label: 'FII 5-Day Trend', dir: pos > last5.length / 2 ? 'bullish' : pos < last5.length / 2 ? 'bearish' : 'neutral', detail: pos + '/' + last5.length + ' buy days' });
      }

      if (globalSentimentData && globalSentimentData.entries && globalSentimentData.entries.length > 0) {
        const g = globalSentimentData.entries[globalSentimentData.entries.length - 1];
        items.push({ label: 'Global Sentiment (GIFT Nifty)', dir: g.giftNifty === 'Bullish' ? 'bullish' : g.giftNifty === 'Bearish' ? 'bearish' : 'neutral', detail: g.giftNifty });
      }

      if (data.NIFTY.signal !== 'WAIT') {
        items.push({ label: 'NIFTY PDH/PDL Signal', dir: data.NIFTY.signal === 'BUY' ? 'bullish' : 'bearish', detail: data.NIFTY.signal });
      }

      if (items.length === 0) return '';

      const bullishCount = items.filter((i) => i.dir === 'bullish').length;
      const bearishCount = items.filter((i) => i.dir === 'bearish').length;
      let verdict, verdictColor;
      if (bullishCount > bearishCount) { verdict = 'Overall Leaning BULLISH'; verdictColor = 'var(--green)'; }
      else if (bearishCount > bullishCount) { verdict = 'Overall Leaning BEARISH'; verdictColor = 'var(--red)'; }
      else { verdict = 'Mixed — No Clear Edge'; verdictColor = 'var(--gold)'; }

      let html = '<div class="premium-card" style="margin-bottom:16px; border: 2px solid var(--gold);">';
      html += '<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">';
      html += '<div class="card-title" style="margin-bottom:0;">✅ Master Pre-Trade Checklist</div>';
      html += '<span class="badge-pill" style="background: rgba(0,0,0,0.2); color:' + verdictColor + ';">' + verdict + '</span>';
      html += '</div>';
      items.forEach((item) => {
        const c = biasColor(item.dir);
        const tick = item.dir === 'neutral' ? '—' : '✓';
        html += '<div class="checklist-item">';
        html += '<span style="color: var(--text);">' + escapeHtml(item.label) + '</span>';
        html += '<span class="checklist-tick' + (item.dir !== 'neutral' ? ' on' : '') + '" style="color:' + c + ';">' + tick + ' ' + escapeHtml(item.detail) + '</span>';
        html += '</div>';
      });
      html += '<div class="timestamp">Combines Gap Score, Alignment, PCR, FII 5-day trend, Global Sentiment, and PDH/PDL signal — for NIFTY. Not investment advice.</div>';
      html += '</div>';
      return html;
    }
    ...
    function renderDailySummary() {
      ...
      html += renderMasterChecklist();
      html += renderLast5DaysTrend(entries);
      html += renderGlobalSentiment();
    ...
    async function initialize() {
      ...
      await Promise.all([loadNews(), loadHolidays(), loadFiiDii(), loadGlobalSentiment()]);
      if (kiteConnected) {
...
// Global Sentiment manual entry — save one day's data (upserts by date)
app.post("/api/global-sentiment", async (c) => {
  try {
    const body = await c.req.json();
    const date = typeof body.date === "string" && body.date ? body.date : indiaDate();
    const validBias = (v: any): "Bullish" | "Bearish" | "Flat" =>
      v === "Bullish" || v === "Bearish" ? v : "Flat";

    const entry: GlobalSentimentEntry = {
      date,
      giftNifty: validBias(body.giftNifty),
      usMarkets: validBias(body.usMarkets),
      asianMarkets: validBias(body.asianMarkets),
      note: String(body.note || "").slice(0, 300),
      createdAt: new Date().toISOString(),
    };

    const existingIdx = globalSentimentEntries.findIndex((e) => e.date === date);
    if (existingIdx >= 0) globalSentimentEntries[existingIdx] = entry;
    else globalSentimentEntries.push(entry);
    globalSentimentEntries.sort((a, b) => a.date.localeCompare(b.date));
    if (globalSentimentEntries.length > GLOBAL_SENTIMENT_MAX_ENTRIES) globalSentimentEntries.shift();

    return c.json({ success: true, entry });
  } catch (err) {
    console.error("[API] Global sentiment save error:", err instanceof Error ? err.message : err);
    return c.json({ error: err instanceof Error ? err.message : "Failed to save global sentiment entry" }, 500);
  }
});

app.get("/api/global-sentiment", (c) => {
  return c.json({ entries: globalSentimentEntries.slice(-14) });
});
