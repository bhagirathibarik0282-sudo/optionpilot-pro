...
function buildTradePlanSection(input: TradeCardInput): string | null {
  ...
  if (isBlocked(input)) return null;
  if (!input.tmPlan) return null;
  if (input.tmPlan.status === "OK") {
    const p = input.tmPlan;
    return (
      `🎯 <b>Trade Plan (forward-test only — NOT backtested):</b>\n` +
      `Entry: <b>₹${p.entry}</b> | SL: <b>₹${p.sl}</b> | T1: <b>₹${p.t1}</b> | T2: <b>₹${p.t2}</b> | T3: <b>₹${p.t3}</b> (100%)\n` +
      `R (risk/unit): ₹${p.rPremium} | Underlying ATR(14): ${p.atrUnderlying} | Delta used: ${p.deltaUsed}\n` +
      `🔁 Trailing: ${p.trailingRule}`
    );
  }
  return `🎯 <b>Trade Plan:</b> UNAVAILABLE (${input.tmPlan.reason || "data insufficient"}) — no numbers fabricated.`;
}

function buildLiveStatusSection(input: TradeCardInput): string | null {
  if (!input.liveStatus) return null;
  const elapsed = input.liveStatus.elapsedMin != null ? `${input.liveStatus.elapsedMin} min since signal` : null;
  return `Status: ${input.liveStatus.statusLine}` + (elapsed ? `\n<i>${elapsed} · forward-test only, not backtested</i>` : "");
}

// Composes the full 7-section trade card. Sections with nothing to show
// (per missing_field_policy: omit) are dropped, never left as an empty
// header. Sections are joined with the SAME separator server.ts's existing
// telegramSplitMessageSafely() splits on, so the existing splitter handles
// length safely without any change to that function.
export function buildTelegramTradeCard(input: TradeCardInput): string {
  const sections: string[] = [];

  sections.push(buildFinalActionSection(input));
  sections.push(buildExactContractSection(input));

  const obe = buildOptionBuyerEdgeSection(input);
  if (obe) sections.push(obe);

  const science = buildMarketScienceSection(input);
  if (science) sections.push(science);

  const devil = buildDevilCheckSection(input);
  if (devil) sections.push(devil);

  const plan = buildTradePlanSection(input);
  if (plan) sections.push(plan);

  const live = buildLiveStatusSection(input);
  if (live) sections.push(live);

  sections.push(`⏰ Time: ${input.timestampIst}`);
