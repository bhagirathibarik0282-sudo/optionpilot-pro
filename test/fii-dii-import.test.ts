...
test("missing Date: line is rejected so stale Drive files can never be stamped as today", () => {
  const text = "FII Cash: 100\nDII Cash: 200";
  const result = parseFiiDiiPasteServerSide(text);
  assert.equal(result, null);
});

test("only FII Cash present is recognized when an explicit source Date is present", () => {
  const result = parseFiiDiiPasteServerSide("Date: 2026-08-20\nFII Cash: 50");
  assert.ok(result);
...
test("all four derivative categories always present in order when a valid dated entry is recognized", () => {
  const result = parseFiiDiiPasteServerSide("Date: 2026-08-20\nDII Cash: 1");
  assert.ok(result);
  assert.deepEqual(
