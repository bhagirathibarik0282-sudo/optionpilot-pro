...
test("selects up to two exact next-expiry CE/PE peer pairs",()=>{
 const r=selectH1LiveContracts(rows,[{symbol:"NIFTY",instrumentToken:99,exchange:"NSE",tradingsymbol:"NIFTY 50",ltp:25031,fetchedAt:new Date().toISOString()}],"2026-09-04");
 assert.equal(r.ready,true);
 assert.deepEqual(r.rows[0].peerPairs.map(x=>[x.expiry,x.strike,x.ceInstrumentToken,x.peInstrumentToken]),[
  ["2026-09-15",25000,5,6],
  ["2026-09-22",25050,7,8],
 ]);
 assert.equal(r.rows[0].peerPairs.some(x=>x.expiry==="2026-09-29"),false);
});
