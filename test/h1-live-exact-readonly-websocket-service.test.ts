...
test("wires Kite-only Greek math cross-check into durable read-only evidence without selector authority", () => {
  ...
  assert.equal(persisted[0].version, "H1_KITE_GREEK_MATH_CROSSCHECK_1M_V3");
  ...
  assert.equal(persisted[0].policyIdentity.referenceImplementationVersion, "H1_KITE_GREEK_MATH_CROSSCHECK_V3");
