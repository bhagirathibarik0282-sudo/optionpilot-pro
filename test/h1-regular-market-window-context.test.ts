...
function utcForIst(hour: number, minute: number): Date {
  const total = hour * 60 + minute - (5 * 60 + 30);
  const h = Math.floor((total + 24 * 60) % (24 * 60) / 60);
  const m = (total + 24 * 60) % 60;
  return new Date(Date.UTC(2026, 8, 4, h, m, 0));
}

test("classifies regular 09:15-15:30 IST window without claiming market open", () => {
