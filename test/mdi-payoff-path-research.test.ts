...
test("bullish path computes observed verified extrema and end payoff",()=>{const out=deriveMdiPayoffPathResearch({...base,path:[p("2026-09-02T04:06:00.000Z",23920,112,94),p("2026-09-02T04:09:00.000Z",23890,96,106),p("2026-09-02T04:18:00.000Z",23960,128,86)]});assert.equal(out.state,"USABLE");assert.equal(out.mdiCoveragePct,100);assert.equal(out.sampleCount,3);assert.ok((out.observedSpotMfePct??0)>0);assert.ok((out.observedSpotMaePct??0)<0);assert.equal(out.observedChosenPremiumMfePct,28);assert.equal(out.observedChosenPremiumMaePct,-4);assert.equal(out.chosenPremiumEndPct,28);assert.equal(out.oppositePremiumEndPct,-14);assert.equal(out.pathObservationPolicy,"OBSERVED_VERIFIED_POINTS_ONLY_NOT_TRUE_CONTINUOUS_EXTREMA");});
