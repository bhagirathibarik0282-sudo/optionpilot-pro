...
test("net payoff current-date rate gate is preserved",()=>{const x=base();x.evaluationDate="2026-09-02";const out=validateFixed30mOptionBuyerPayoff(x);assert.equal(out.state,"UNAVAILABLE");assert.ok(out.blockers.some(v=>v.includes("RATE_SCHEDULE_NOT_VERIFIED_FOR_EVALUATION_DATE")));});
