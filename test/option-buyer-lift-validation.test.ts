...
test("compares same selector-qualified baseline against MDI-matched subset by DTE",()=>{const out=validateOptionBuyerMdiLift([sample("a",true,"CE",1,115),sample("b",false,"CE",1,95),sample("c",true,"CE",3,110)]);assert.equal(out.state,"USABLE");assert.equal(out.baselineQualifiedCount,3);assert.equal(out.mdiFilteredQualifiedCount,2);assert.ok(Math.abs((out.overallMdiRetentionPct??0)-66.66666666666666)<1e-9);const expiry=out.buckets.find(x=>x.dteBucket==="EXPIRY_0_1");assert.equal(expiry?.baselineCount,2);assert.equal(expiry?.mdiFilteredCount,1);assert.equal(expiry?.mdiRetentionPct,50);assert.ok((expiry?.avgNetReturnLiftPctPoints??0)>0);assert.equal(out.semantics,"RESEARCH_FILTER_LIFT_ONLY_NOT_CAUSAL_EDGE_PROOF");assert.equal(out.regimePolicy,"NOT_INCLUDED_UNTIL_DETERMINISTIC_REGIME_SOURCE_IS_BOUND");assert.equal(out.inferencePolicy,"DESCRIPTIVE_ONLY_NO_MIN_SAMPLE_OR_STATISTICAL_SIGNIFICANCE_CLAIM");});
...
test("no live authority and no statistical inference claim",()=>{const out=validateOptionBuyerMdiLift([]);assert.equal(out.inferencePolicy,"DESCRIPTIVE_ONLY_NO_MIN_SAMPLE_OR_STATISTICAL_SIGNIFICANCE_CLAIM");assert.equal(out.affectsVerdict,false);assert.equal(out.affectsTelegram,false);assert.equal(out.affectsExecution,false);assert.equal(out.createsOrders,false);assert.equal(out.aiMayOverride,false);});
