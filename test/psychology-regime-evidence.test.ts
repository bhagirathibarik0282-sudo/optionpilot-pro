...
test("valid deterministic upstream regime evidence is accepted", () => {
  ...
  ], "2026-08-20T09:21:00+05:30", ["TREND", "EXPIRY"]);
...
test("lookahead regime evidence is rejected", () => {
  ...
  ], "2026-08-20T09:21:00+05:30", ["REVERSAL"]);
  assert.equal(result.valid, false);
...
test("duplicate regime evidence is rejected", () => {
  ...
  ], "2026-08-20T09:21:00+05:30", ["HIGH_IV"]);
  assert.equal(result.valid, false);
...
test("blank rule version is rejected", () => {
  ...
  ], "2026-08-20T09:21:00+05:30", ["GAP"]);
  assert.equal(result.valid, false);
  assert.ok(result.blockers.includes("REGIME_EVIDENCE_RULE_VERSION_MISSING:GAP"));
});

test("proven regime must also be present in diagnostic regime labels", () => {
  const result = validateShadowValidationRegimeEvidence([
    { regime: "EXPIRY", source: "DETERMINISTIC_UPSTREAM", observedAt: "2026-08-20T09:20:00+05:30", ruleVersion: "EXPIRY_RULE_V1" },
  ], "2026-08-20T09:21:00+05:30", ["TREND"]);
  assert.equal(result.valid, false);
  assert.ok(result.blockers.includes("REGIME_EVIDENCE_LABEL_MISMATCH:EXPIRY"));
});
