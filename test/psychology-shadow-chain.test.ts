...
import { runPsychologyShadowChain } from "../psychology-shadow-chain.ts";
...
const base = {
  ...
  triggerInput: {
    ...
    currentFingerprint: "S17:HOLD:RESPONDING_WELL:BUYERS_IN_CONTROL",
...
test("complete deterministic chain reaches HOLD and can speak", () => {
  ...
  assert.match(r.coach.heading, /SCALP • NIFTY 24900 CE • HOLD • S17/);
});

test("duplicate message is suppressed end-to-end", () => {
  const r = runPsychologyShadowChain({ ...base, triggerInput: { ...base.triggerInput, lastSpokenFingerprint: base.triggerInput.currentFingerprint } });
  assert.equal(r.trigger.shouldSpeak, false);
  assert.equal(r.coach.shouldSpeak, false);
});

test("stale premium evidence becomes urgent DATA_UNAVAILABLE without trade authority", () => {
  const r = runPsychologyShadowChain({ ...base, premiumEvidence: { ...base.premiumEvidence, dataFresh: false }, triggerInput: { ...base.triggerInput, currentFingerprint: "S17:DATA_UNAVAILABLE" } });
...
test("candidate-scoped fingerprint mismatch fails closed end-to-end", () => {
  const r = runPsychologyShadowChain({ ...base, triggerInput: { ...base.triggerInput, currentFingerprint: "W04:HOLD" } });
  assert.equal(r.trigger.shouldSpeak, false);
  assert.equal(r.trigger.reason, "FINGERPRINT_CANDIDATE_SCOPE_MISMATCH");
  assert.equal(r.coach.shouldSpeak, false);
});
