...
test("runtime storage wiring declares Kite source and keeps index receipt proxy explicit", () => {
  assert.ok(wire.includes('sourceProvider: \\\"KITE\\\"'));
  assert.ok(wire.includes('KITE_INDEX_SNAPSHOT_RECEIPT_PROXY'));
});

test("runtime futures use exact patched receipt when present and retain fallback", () => {
  assert.ok(wire.includes('receivedAt: future.receivedAt ?? storageReceivedAt'));
  assert.ok(wire.includes('KITE_FUTURES_LIVE_CONTRACT_MASTER|QUOTE_RESPONSE_RECEIPT'));
  assert.ok(wire.includes('KITE_FUTURES_SNAPSHOT_RECEIPT_PROXY'));
  assert.ok(wire.includes('tradingSymbol: future.tradingsymbol ?? null'));
});

test("runtime option rows retain LIVE_CONTRACT_MASTER provenance", () => {
...
test("runtime chain context does not invent provider source timestamp", () => {
  assert.ok(wire.includes('sourceTimestamp: null'));
});

test("runtime wiring states no extra Kite request or trading side effect", () => {
...
test("current server has exactly one runtime snapshot anchor for safe patching", () => {
  assert.equal(server.split(snapshotAnchor).length - 1, 1);
});
